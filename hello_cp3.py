...
def show_photo(filename):
    ...
    if request.method == 'GET':
        if filename is None:
            ...
            #image = T.Resize(500,500)(os.path.join(file_dir, '%s' % filename))
        ...
        pass
def vis_results(class_name, dets, thresh=0.8):
    predictions = []
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            rect = {"BoxList": [str(i) for i in np.rint(bbox[:4]).astype(int)]}
            rect["BoxList"].append(class_name)
...
def process(data):
    ...
    new_mult = []
    new_single = {}
    for info in data['im_info']:
        print("+++++++++++++++++++++++++")
        print("info",info)
        print("+++++++++++++++++++++++++")
        print("data['im_info'][info]", data['im_info'][info])
        info_value = {}
        info_value['filename'] = data['im_info'][info]['filename']
        info_value['height'] = data['im_info'][info]['height']
        info_value['width'] = data['im_info'][info]['width']
        info_value['predictions'] = []
        for pred in data['im_info'][info]['predictions']:
            for box in pred:
                info_value['predictions'].append(box)
        print('++++++++++++++++++++++++++++++++++')
        print('info_value',info_value)
        new_single[info] = info_value
        print('++++++++++++++++++++++++++++++++++')
        print('new_single[info]', new_single[info])
        # new_single[info]['filename'] =  data['im_info'][info]['filename']
        # new_single[info]['height'] = data['im_info'][info]['height']
        # new_single[info]['width'] = data['im_info'][info]['width']
        # new_single[info]['predictions'] = []
        # for pred in data['im_info'][[info]['predictions']]:
        #     for box in pred:
        #         new_single[info]['predictions'].append(box)
        new_data['im_info']=new_single
    # new_data['im_info'] = new_mult
    return new_data
